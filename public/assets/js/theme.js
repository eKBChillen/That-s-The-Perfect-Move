...
var getColor = function getColor(name) {
  ...
  return getComputedStyle(dom).getPropertyValue("--baikal-".concat(name)).trim();
};
...
/*                              Animated Swiper                               */
/* -------------------------------------------------------------------------- */
var animatedSwiperInit = function animatedSwiperInit() {
  var swiperContainer = document.querySelectorAll('.animated-swiper');
  if (swiperContainer.length) {
    swiperContainer.forEach(function (swiper) {
      var swiperInstance = swiper.querySelector('[data-swiper]').swiper;
...
var bgPlayerInit = function bgPlayerInit() {
  ...
  var adjustBackgroundRatio = function adjustBackgroundRatio() {
    ...
    ytElements.forEach(function (ytEl) {
      var ytElement = ytEl;
      var width = ytElement.parentElement.offsetWidth + 400;
      var height = width * 9 / 16;
      var minHeight = ytElement.parentElement.offsetHeight + 400;
...
var navbarInit = function navbarInit() {
  var navbars = document.querySelectorAll('[data-navbar-soft-on-scroll]');
  if (!navbars.length) return;
  navbars.forEach(function (navbar) {
    var windowHeight = window.innerHeight;
    var togglerButton = navbar.querySelector('[data-toggler-button]');
    var classList = navbar.getAttribute('class').split(' ');
    var breakpoint = classList.find(function (item) {
      return item.includes('navbar-expand-');
    }).split('navbar-expand-')[1];
    var bgNavbarRGB = '11, 23, 39';
    var alpha;
    var setNavbarBackground = function setNavbarBackground() {
      var isCollapsed = togglerButton.classList.contains('collapsed');
      alpha = Math.min(Math.max(window.scrollY / windowHeight * 2, 0), 1);
      var isSmallScreen = window.innerWidth <= utils.breakpoints[breakpoint];
      if (isSmallScreen && alpha <= 0.2 && !isCollapsed) {
        navbar.style.backgroundColor = "rgba(".concat(bgNavbarRGB, ", ").concat(Math.min(alpha + 0.2, 1), ")");
      } else {
        navbar.style.backgroundColor = "rgba(".concat(bgNavbarRGB, ", ").concat(alpha, ")");
      }
    };
    var handleTogglerButtonClick = function handleTogglerButtonClick() {
      var isCollapsed = togglerButton.classList.contains('collapsed');
      if (alpha <= 0.2) {
        navbar.style.backgroundColor = isCollapsed ? "rgba(".concat(bgNavbarRGB, ", 0)") : "rgba(".concat(bgNavbarRGB, ", 0.2)");
      }
    };
    var handleWindowResize = function handleWindowResize() {
      if (window.innerWidth > utils.breakpoints[breakpoint]) {
        navbar.style.backgroundColor = alpha === 0 ? "rgba(".concat(bgNavbarRGB, ", 0)") : "rgba(".concat(bgNavbarRGB, ", ").concat(alpha, ")");
      } else if (!togglerButton.classList.contains('collapsed') && alpha <= 0.2) {
        navbar.style.backgroundColor = "rgba(".concat(bgNavbarRGB, ", 0.2)");
      }
    };
    setNavbarBackground();
